...
### 列配置
- **功能**：添加/删除具有不同数据类型（文本、数字、日期、布尔值、选择）的列
- **目的**：允许定制数据结构以匹配特定用例
- **触发**：点击列标题菜单或"添加列"按钮
- **流程**：点击添加列 → 选择数据类型 → 输入列名称 → 配置选项 → 保存 → 列显示在表格中
- **成功标准**：列保持其类型验证并正确显示
...
### 数据导出
- **功能**：将表格数据导出为JSON格式以供外部使用
- **目的**：允许用户提取其数据用于备份或集成目的
- **触发**：点击表格工具栏中的导出按钮
- **流程**：点击导出 → 选择格式 → 生成下载 → 文件下载到设备
- **成功标准**：导出的数据保持完整性，可以重新导入
